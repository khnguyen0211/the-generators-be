...
class TextToVideoService:
    """Service for text-to-video generation. Uses strategy pattern."""
    ...
    def generate(self, payload: dict) -> dict:
        """Generate video using selected provider strategy."""
        provider = payload.get("provider", self._config.text_to_video_default)
    ...
    def get_available_models(self) -> dict:
        ...
        available = []
        for provider in self._config.text_to_video_models:
            if provider in self.STRATEGIES:
                config = self._config.get_text_to_video_config(provider)
                available.append({
                    "provider": provider,
                    "model": config.get("model", ""),
                    "is_default": provider == self._config.text_to_video_default,
                })
